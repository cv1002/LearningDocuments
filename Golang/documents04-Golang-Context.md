...
- [Context](#context)
  ...
  - [context 底层实现原理](#context-底层实现原理)
    ...
    - [结构体](#结构体)
      ...
      - [timerCtx](#timerctx)
      - [valueCtx](#valuectx)
  - [如何使用 context](#如何使用-context)
    - [传递共享的数据](#传递共享的数据)
    - [取消 goroutine](#取消-goroutine)
    - [防止 goroutine 泄漏](#防止-goroutine-泄漏)
  - [context 真的这么好吗](#context-真的这么好吗)
  - [总结](#总结)
  - [参考资料](#参考资料)
...
| 类型                    | 分类   | 作用                                     |
| ----------------------- | ------ | ---------------------------------------- |
| context.Context         | 接口   | 上下文接口，定义了 Context 的基本方法    |
| context.cancelCtx       | 结构体 | 实现了 Context 接口，并添加了取消功能    |
| context.timerCtx        | 结构体 | 继承了 cancelCtx，并添加了超时功能       |
| context.valueCtx        | 结构体 | 继承了 Context 接口，并添加了 k-v 功能   |
| context.emptyCtx        | 结构体 | 实现了 Context 接口，但不包含任何功能    |
| context.cancelFunc      | 函数   | 取消函数，用于取消 Context               |
| context.Background      | 函数   | 返回一个空的 Context                     |
| context.TODO            | 函数   | 返回一个空的 Context                     |
| context.WithCancel      | 函数   | 返回一个可取消的 Context                 |
| context.WithDeadline    | 函数   | 返回一个可取消的 Context，并设置截止时间 |
| context.WithTimeout     | 函数   | 返回一个可取消的 Context，并设置超时时间 |
| context.WithValue       | 函数   | 返回一个可取消的 Context，并设置 k-v 值  |
| context.newCancelCtx    | 函数   | 返回一个可取消的 Context                 |
| context.propagateCancel | 函数   | 向下传递 Context 节点之间的取消关系      |
| context.parentCancelCtx | 函数   | 找到第一个可取消的 Context               |
| context.removeChild     | 函数   | 去掉父节点的孩子节点                     |
| context.init            | 函数   | 包初始化                                 |
...
3. The same Context may be passed to functions running in different goroutines; Contexts are safe for simultaneous use by multiple goroutines.
