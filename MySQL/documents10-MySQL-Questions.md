...
- [MySQL InnoDB 引擎加索引的时候会锁表吗](#mysql-innodb-引擎加索引的时候会锁表吗)
- [事务执行中，mysql如果宕机了，重新恢复会发生什么？](#事务执行中mysql如果宕机了重新恢复会发生什么)
...
# MySQL InnoDB 引擎加索引的时候会锁表吗
MySQL的InnoDB引擎在加索引时不一定锁表，这取决于MySQL的版本和使用的在线DDL技术，例如在 MySQL 5.6 及之后版本，通常可以通过ALGORITHM=INPLACE和LOCK=NONE选项实现几乎无锁的索引创建。然而，在某些情况下，如存在未提交的事务或执行的是需要表重建的操作，仍然可能出现短暂的锁定。

影响是否锁表的因素
- MySQL版本：
  - MySQL 5.6 之前: 大部分情况下会执行排它锁定，导致数据阻塞
  - MySQL 5.6 及之后: 引入了在线DDL技术，显著减少了锁定时间，允许在创建索引时并发执行DML操作
- 在线DDL模式：
  - ALGORITHM=INPLACE: 允许在表数据不变的情况下修改元数据，是实现无锁操作的核心
  - LOCK=NONE: 在INPLACE模式下，指定此选项可以避免对表的写锁定，实现并发访问

操作的类型：
- 某些操作，如添加可为空的列，可能只会导致短暂的行锁定，而非全程锁表。
- 全表重建或涉及大量数据量的操作可能仍然需要较长的锁定时间，即使在在线模式下。

其他事务：
- 如果在执行索引添加操作时，表上存在未提交的事务，InnoDB会等待这些事务完成后才能开始索引构建。

如何最大程度避免锁表
- 升级MySQL版本: 确保使用MySQL 5.6或更高版本
- 使用在线DDL选项: 在ALTER TABLE语句中指定ALGORITHM=INPLACE, LOCK=NONE
- 在低峰时段执行: 选择业务量较低的时段进行操作，以减少潜在的影响
- 充分测试: 在生产环境执行前，进行充分的测试，并制定备份和回滚计划

# 事务执行中，mysql如果宕机了，重新恢复会发生什么？
